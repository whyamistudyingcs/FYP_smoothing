...
def roberta_noise_forward(
    ...
    for i, layer_module in enumerate(self.layer):
        ...
        if i % self.config.nth_layers == 0 and not self.config.single_layer:
            noise = torch.randn_like(layer_outputs[0]) * self.config.noise_eps
            hidden_states = layer_outputs[0] + noise
        else:
            hidden_states = layer_outputs[0]
